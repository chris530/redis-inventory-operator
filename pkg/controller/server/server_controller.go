...
import (
	...
	devopsv1 "github.com/redis-inventory-operator/redis-inventory-operator/pkg/apis/devops/v1"
...
func (r *ReconcileServer) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	pod := newPodForCR(instance)
        ...
        match, _ := regexp.MatchString("^"+os.Getenv("REDIS_SEARCH"), pod.Name )

        if match { 
          client := redis_init()
          fmt.Println("Adding ",pod.Name," to redis inventory")
          client.Set(pod.Name, pod.Namespace, 0)
        }
